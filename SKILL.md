...
whose context is the cost the tiering exists to contain. A session has one model, set at start, and the project
...
|---|---|---|---|
| "Start a new Solowright project" → scaffold from `assets/`, committed | Claude Code, **the first spec session** | opens at the session tier; the person switches it to the top tier as for any spec session | the person and the session |
| Idea conversation | same session | Fable | the person and Claude |
| Constitution → `CLAUDE.md` filled in | same session, committed | Fable | the person and Claude |
| First spec → `spec.md` | same session | Fable | the person and Claude |
...
0. **Scaffold, from the skill's own templates.** The person creates an
   ...
   opens at the session tier and asks for the top tier like any other
   — and before any conversation it writes the project's skeleton
